...
{
    return (0.5f * projection[3][2]) / (depth + 0.5f * projection[2][2] - 0.5f);
}
...
vec3 SphericalFibonacci(float i, float n)
{
    ...
    float mf = i * (C_PHI - 1.0f) + (-floor(i*(C_PHI - 1.0f)));
	float phi = 2.0f * C_PI * mf;
...
vec2 OctWrap(vec2 v)
{
    return (1.0f - abs(v.yx)) * (all(greaterThan(v.xy, vec2(0.0f))) ? 1.0f : -1.0f);
}
